...
HWND window_wnd_get_from_ptr(windows_win* windows, HWND hwnd)
{
    const size_t windows_count = sizeof(windows->windows_wnds) / sizeof(windows->windows_wnds[0]);
    size_t index = 0;
    for (size_t i = 0; i < windows_count; ++i)
    {
        index = i * (windows->windows_wnds[i] == hwnd);
    }
    printf("HNWD found at index %zu\n", index);
    return windows->windows_wnds[index];
}
...
void window_add(windows_win* windows, const char* name, int x, int y, int width, int height)
{
    ...
    windows->windows_wnds[index] = CreateWindowEx(WS_EX_APPWINDOW, name, name, normal, x, y, width, height, NULL, NULL, windows->windows_modules[index], NULL);
...
bool window_show_all(windows_win* windows)
{
    ...
    for (size_t i = 0; i < windows_count; ++i)
    {
        ...
        if (PeekMessage(&windows->windows_msgs[i], NULL, 0, 0, PM_REMOVE))
        {
            quit_count += (windows->windows_msgs[i].message == WM_QUIT);
